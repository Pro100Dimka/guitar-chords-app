...
export const Waveform = ({
  ...
  const refresh = useMemo(
    () => Math.floor(bufferId / REFRESH_FRAMES),
    [bufferId]
  );
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const alignedAudio = useMemo(
    () => getAlignedAudio(audioBuffer, 2048),
    [refresh]
  );
  ...
  useEffect(() => {
    ...
    progress.value = withTiming(1, {
      duration: (1000 * REFRESH_FRAMES) / bufPerSec
    });
...
const getWaveformPath = (samples: number[], width: number, height: number) => {
  ...
  const subsample =
    samples.length > MIN_SUBSAMPLE_SIZE ? WAVEFORM_SUBSAMPLE : 1;
