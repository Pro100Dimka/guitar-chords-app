export default {
  en: "Англійська",
  ru: "Російська",
  ua: "Українська",
  headerTitle: "Гітарні аккорди",
  searchPlaceholder: "Пошук аккордів...",
  home: "Головна",
  Search: "Пошук",
  library: "Бібліотека",
  Create: "Створити",
  NewSong: "Нова пісня",
  SongTitle: "Пісня",
  Band: "Гурт/Артист",
  createSong: "Створити пісню",
  ChordsLyrics: "Аккорди / текст",
  EnterSongTitle: "Введіть назву пісні та виконавця",
  SaveSong: "Зберегти пісню",
  FieldRequired: "Обов'язкове поле",
  Edit: "Редагувати",
  SongLink: "Пісня",
  TrainingLink: "Тренінг",
  Close: "Закрити",
  InvalidUrl: "Невірний URL",
  Delete: "Видалити",
  ElementChanged: "Елемент змінено",
  ElementDeleted: "Елемент видалено",
  ElementAdded: "Елемент збережено",
  ErrorSavingSong: "Error saving song",
  NoSongs: "Немає пісень",
  guitar: "Гітара",
  chromatic: "Кроматична",
  instrument: "Інструмент",
  mode: "Режим",
  gtr_string: "Струна",
  reference: "Посилання",
  feedback: "Зворотній зв'язок",
  more_settings: "Більше налаштувань...",
  no_tone: "Немає тону",
  language: "Мова",
  theme: "Тема",
  dark: "Темна",
  close: "Закрити",
  settings: "Налаштування",
  reference_a4: "Посилання (A4)",
  tuning_440: "440Hz (standard)",
  tuning_432: "432Hz (Verdi)",
  tuning_444: "444Hz (high pitch)",
  graphics: "Графіка",
  graphics_high: "Збільшити графіку",
  graphics_low: "Зменшити графіку",
  error_mic_access:
    "Доступ до мікрофона заборонено. Переходіть до налаштувань і увімкніть доступ до мікрофона.",
  configure_permissions: "Налаштування дозволів"
};
